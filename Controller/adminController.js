...
const loginVerify=async(req,res)=>{
    ...
    if(adminData)
    {
        ...
        if(passwordMatch)
        {
            ...
            console.log("session is : ", req.session," adminData._id is :", adminData._id)
...
const updateUser=async(req,res)=>{
    try{
        ...
        const userData=await User.findByIdAndUpdate({_id:id},{$set: {name: name, address:address, email:email, mobile:mobile}})
        res.redirect('/admin/user')
...
const addNewproduct=async(req,res)=>{
    try {
        ...
        if (!/^\d+(\.\d{1,2})?$/.test(priceValue) || parseFloat(priceValue) <= 0) {
          extension=false
          const message ='Price must be a positive number with up to two decimal places.'            
          return  res.render('adminAlert', { message });     
...
const updateProduct=async(req,res)=>{
    try {
      ...
      // console.log(req.body,"vbnmbvbnm",req.body.images ,req.files,req.file)
        ...
        const productData=await Product.findByIdAndUpdate({_id:id},{$set: {name: name, price:price, description: description, category:category}})
        res.redirect('/admin/product') 
...
const loadOrders=async(req,res)=>{
    try {
        ...
        const order= await Order.find({}).sort({date:-1}).populate('user_id').lean()
       ...
       // console.log("Order from 3 is ", order[3])
        ...
        res.render('order',{order:order,ord_details:formattedOrders,message:message})
    } catch (error) {
        console.log(error.message)
...
const editOrder=async(req,res)=>{
    try {
        ...
        const order=await Order.findOne({_id:orderid}).populate('user_id').populate('product.pro_id').lean()
...
const changeStatus =async(req,res)=>{
    try {
        ...
        {
            const orderData= await Order.findOne({_id:orderid}).populate('product.pro_id').lean()
...
const deleteCoupon = async (req, res) => {
  ...
  const loadDashboard = async (req, res) => {
    try {
      ...
      const totalRevenue = allOrders.reduce((totall, order) => totall + Number(order.amount), 0);
      ...
      const catsales= await Order.aggregate([
        ...
        },
        {
          $group: {
            ...
            totalSales: { $sum: { $multiply: ['$product.qty', '$product.price'] } }, // Calculate total sales
          },
        },
      ])
      .exec();
...
const dailySalesdownloadpdf=(req,res)=>{
 ...
 //  doc.pipe(fs.createWriteStream("output.pdf"))
   console.log(dailyorders)
    ...
    //       .text(`orderDate: ${order.date}`,{width:15})
    //       .text(order._id)
    //       .moveDown()      
    //   });
